 ...
 * Note: Module-level state - use resetProcessCache() in test teardown
...
// Note: MIN_CALL_INTERVAL is intentionally lower than StatusBar's refetchInterval (10s)
// to act as a safety net for unexpected rapid calls from other sources
const MIN_CALL_INTERVAL = 5000; // 5 seconds minimum between subprocess calls

/**
 * Reset the process cache. Useful for testing.
 */
export function resetProcessCache(): void {
  processCache = { value: false, timestamp: 0 };
  lastCallTimestamp = 0;
}
 ...
 * Includes caching and rate limiting to reduce subprocess overhead.
...
export async function isProcessRunning(): Promise<boolean> {
  ...
  // Layer 1: Rate Limiting (only if cache is valid)
  if (now - lastCallTimestamp < MIN_CALL_INTERVAL && processCache.timestamp > 0) {
    logger.debug('[ProcessCheck] Rate limit hit, returning cached value');
    return processCache.value;
  }

  // Layer 2: Cache Check
  if (now - processCache.timestamp < CACHE_TTL && processCache.timestamp > 0) {
  ...
  try {
    ...
    // Filter to find main processes
    const mainProcesses: ProcessInfo[] = [];
